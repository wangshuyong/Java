        ...
        for(int i=0; i < list.size(); ++i){
            ...
            if(firstColumn.equals("")){
            	...
            	if(firstColumn.trim().length()>11){
                	importMessage = "第"+ index + "行，第1列“手机”号码格式错误，导入失败,请修正后重新导入！！！";
                	break;
                }else{
                    if(this.exists(firstColumn)){
                    	importMessage = "第"+ index + "行，第1列"+firstColumn+"已存在，导入失败,请修正后重新导入！！！";
                    	break;
                    }
            ...
            if(secondColumn.equals("")){
	            ...
	            if(secondColumn.length() > 30){            //长度过大
	            	importMessage = "第"+ index + "行，第2列“姓名”长度大于20，导入失败,请修正后重新导入！！！";
	                break;
	            }
            ...
            if(thirdColumn.equals("")){
                ...
                if((!thirdColumn.equals("男"))&&(!thirdColumn.equals("女"))){            //长度过大
                	importMessage = "第"+ index + "行，第3列“性别”填写错误，导入失败,请修正后重新导入！！！";
                    break;
                }
            ...
            if(forthColumn.equals("")){
                ...
                if(Integer.parseInt(forthColumn) < 0){            //长度过大
                	importMessage = "第"+ index + "行，第4列“年龄”填写错误，导入失败,请修正后重新导入！！！";
                    break;
                }
            ...
            if(fifthColumn.equals("")){
                ...
                if (!m.find()){
                	importMessage = "第"+ index + "行，第5列“邮箱”不符合标准，导入失败,请修正后重新导入！！！";
                    break;
            	}
